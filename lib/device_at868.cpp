...
static const char*	class_name = "DeviceAT868";

DeviceAT868::DeviceAT868(ObjectManager& _manager)
: DeviceModbus(_manager, OBJECT_TYPE_DEV_AT868, true), correction_interval_(1)
{
...
DeviceAT868::DeviceAT868(ObjectManager& _manager, JSONNode const& _properties)
: DeviceModbus(_manager, OBJECT_TYPE_DEV_AT868, true), correction_interval_(1)
{
	TRACE_INFO("AT868 Created");
...
const char*	DeviceAT868::GetClassName()
{
	return	class_name;
}

bool	DeviceAT868::ReadValue(std::string const& _epid, time_t& _time, std::string& _value)
{
	try
	{
...
std::string* DeviceAT868::strSplit(std::string strOrigin, std::string strTok, int size)
{
	int cutAt;
	int index = 0;
...
double DeviceAT868::cal_data(double mantissa, double exponent)
{
...
void	DeviceAT868::Preprocess()
{
	DeviceModbus::Preprocess();
}

void	DeviceAT868::Process()
{
	uint8_t slave_id = 0;
	float p = 0;
	...
	std::string* temp;
	size_t ex_size = 0;	
	uint8_t serial_command[AT868_SENSOR_COUNT][12] = {
	...
	if(correction_timer_.RemainTime() == 0)
	{
		...
		TRACE_INFO("AT868 read data");
#if 1
		for(uint8_t i = 0 ; i < AT868_SENSOR_COUNT ; i++)
		{
		...
		TRACE_INFO("AT868 Remain Time : " << correction_timer_.RemainTime());
...
////////////////////////////////////////////////////////////////////////////////
const char*	DeviceAT868::Type()
{
	return	OBJECT_TYPE_DEV_AT868;
}
