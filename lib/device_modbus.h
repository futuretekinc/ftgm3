...
protected:
			bool    ReadHoldingRegisters(uint16_t slave_id, uint16_t address, int16_t* values, uint16_t count);	
			...
			bool	RequestAndResponse(uint8_t* _request, uint32_t _request_len, uint8_t* _response, uint32_t _max_response_len, uint32_t& _response_len, uint32_t _timeout);
