...
RequestFrame::RequestFrame(uint8_t _device_id, FunctionCode _function, uint16_t _address, uint16_t _value)
{
	switch(_function)
	{
	case	READ_HOLDING_REGISTERS:
		{
			frame_.read_holding_registers.device	= _device_id;
			...
			uint16_t	crc = modbus::crc16(frame_.raw, 6);
			frame_.read_holding_registers.crc		= crc;
			frame_size_ = 8;
	...
	case	WRITE_SINGLE_REGISTER:
		{
			frame_.write_single_register.device		= _device_id;
			...
			uint16_t	crc = modbus::crc16(frame_.raw, 6);
			frame_.write_single_register.crc		= crc;
...
ResponseFrame::ResponseFrame(uint8_t _device_id, FunctionCode _function, uint8_t* _buffer, uint32_t _length)
: valid_(false)
{
	...
	else if (_buffer[1] != _function)
	{
		TRACE_ERROR("Function code mismatch[" << _function << " , " << _buffer[1] << "]");
	}
	else
	{
		frame_size_ = _length;
		...
		case	READ_HOLDING_REGISTERS:
			{
				if (frame_size_ >= (5 + frame_.read_holding_registers.count))
				{
					uint16_t	crc = modbus::crc16(frame_.raw, 3 + frame_.read_holding_registers.count);
					uint16_t	frame_crc = frame_.raw[3+frame_.read_holding_registers.count]+ ((uint16_t)frame_.raw[3+frame_.read_holding_registers.count + 1] << 8);
				...
				{
					TRACE_ERROR("Frame Size : " << frame_size_);
					TRACE_ERROR("Count : " << frame_.read_holding_registers.count);
				}
			}			
			break;
		...
		case	WRITE_SINGLE_REGISTER:
			{
				...
				{
					uint16_t	crc = modbus::crc16(frame_.raw, 6);
					uint16_t	frame_crc = frame_.raw[6] + ((uint16_t)frame_.raw[7] << 8);
...
uint16_t	modbus::crc16(void*	data, uint32_t	len)
{
	uint8_t		temp;
	uint16_t	crc = 0xFFFF;
