#ifndef	SERVER_LINKER_KAFKA_H_
#define	SERVER_LINKER_KAFKA_H_
...
class	ServerLinkerKafka : public ProcessObject
{
public:
	...
	{
	public:
					EventCB(ServerLinkerKafka& _linker);

			void 	event_cb (RdKafka::Event &_event) ;

	protected:
		ServerLinkerKafka&	linker_;
	};

	class DeliveryReportCB : public RdKafka::DeliveryReportCb, Object
	{
	public:
					DeliveryReportCB(ServerLinkerKafka& _linker);
	...
	{
	public:
		Link(ServerLinkerKafka& _linker, std::string const& _topic_name, int32_t _partition = DEFAULT_CONST_MSG_PARTITION);
...
const	std::string&	GetTopic()	{ return	topic_name_;	};
	...
	{
	public:
					UpLink(ServerLinkerKafka& _linker, std::string const& _topic_name, int32_t _partition = DEFAULT_CONST_MSG_PARTITION);
	...
	public:
		...
		{
		public:
			ConsumeCB(ServerLinkerKafka::DownLink& _link) : ConsumeCb(), link_(_link) {};
		...
		protected:
			...
			DownLink&	link_;
					...
					DownLink(ServerLinkerKafka& _linker, std::string const& _topic_name, int32_t _partition = DEFAULT_CONST_MSG_PARTITION);
	...
	{
	public:
		friend	class	ServerLinkerKafka;
		ConsumeCB(ServerLinkerKafka& _linker) : ConsumeCb(), linker_(_linker) {};
	...
	protected:
		...
		ServerLinkerKafka&	linker_;
						...
						ServerLinkerKafka(ObjectManager* _manager= NULL);
						~ServerLinkerKafka();
